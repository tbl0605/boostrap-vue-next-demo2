<template>
  ...
  >
    ...
    >
      <slot :field="field" :componentField="componentField">
        <input
          class="default-input"
          v-bind="field"
          :value="text || ''"
    ...
    <transition name="vue-simple-suggest">
      ...
      >
        <li v-if="!!$slots['misc-item-above']" :class="styles.miscItemAbove">
          <slot name="misc-item-above" :suggestions="suggestions" :query="text"></slot>
        </li>
        ...
        <li v-if="!!$slots['misc-item-below']" :class="styles.miscItemBelow">
          <slot name="misc-item-below" :suggestions="suggestions" :query="text"></slot>
        </li>
      </ul>
    </transition>
  </div>
      ...
      handler(current) {
        this.filterResult =
          current != null
            ...
            : (el, value) => {
                return value
                  ? ~display(el, this.displayAttribute).toLowerCase().indexOf(value.toLowerCase())
                  : true;
              };
      },
      immediate: true
  ...
  computed: {
    ...
    textLength() {
      return (
        ...
        (this.inputElement && this.inputElement.value.length) ||
        0
      );
    },
    isSelectedUpToDate() {
  ...
  methods: {
    ...
    async getSuggestions(value) {
      ...
      try {
        ...
        nextIsPlainSuggestion =
          (typeof result[0] !== 'object' && typeof result[0] !== 'undefined') ||
